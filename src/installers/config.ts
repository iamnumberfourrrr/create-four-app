...
const TEMPLATE_DIR = fileURLToPath(new URL("../templates/config", import.meta.url));

const configInstaller: Installer = {
  name: "config",
  ...
  async install(ctx: InstallerContext): Promise<FileOp[]> {
    ...
    const dest = join(ctx.projectDir, "packages", "config");
...
register(configInstaller);

export { configInstaller };
